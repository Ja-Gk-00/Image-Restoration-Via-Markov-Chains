...
def generate_checkerboard(width, height, num_tiles=8):
    ...
    for i in range(num_tiles):
        for j in range(num_tiles):
            if (i + j) % 2 == 0:
                board[i*tile_h:(i+1)*tile_h, j*tile_w:(j+1)*tile_w] = 255
    return board
...
def main():
    parser = argparse.ArgumentParser(description='Generate black-and-white pattern images without PIL.')
    parser.add_argument('--output_dir', '-o', type=str, required=True,
                        help='Directory to save generated images')
    parser.add_argument('--resolution', '-r', nargs=2, type=int, required=True,
