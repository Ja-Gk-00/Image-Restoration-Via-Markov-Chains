...
@numba.njit
def _sample_neigh_quadratic(
    y_old, neigh, x_obs, lambda_r, beta_prior, beta_t, alpha=99999.0
):
    vmin = np.min(neigh)
    vmax = np.max(neigh)
    ...
    for idx in range(m):
        v = cand[idx]
        energies[idx] = lambda_r * (v - x_obs) ** 2 + beta_prior * np.sum(
            np.minimum((neigh - v) ** 2, alpha)
        )
    e0 = energies.min()
...
class GibbsSampler:
    ...
    def fit(self, shuffle_pixels=False, parallel_channels=False, binary_map=False):
        ...
        if parallel_channels:
            ...
            def run_channel(c):
                ...
                for t in range(1, self.num_iter + 1):
                    ...
                    if self.verbose and t % 10 == 0:
                        ...
                        print(
                            f"[Channel {c}][Iter {t}] Loss={loss_val:.2f}, Energy={energy_val:.2f}"
                        )
                # save last sample
            ...
            with ThreadPoolExecutor(max_workers=C) as ex:
                results = [
                    f.result() for f in [ex.submit(run_channel, c) for c in range(C)]
                ]
            ...
            for t in range(1, self.num_iter + 1):
                coords = [
                    (c, i, j) for c in range(C) for i in range(H) for j in range(W)
                ]
                if shuffle_pixels:
                    np.random.shuffle(coords)
                for c, i, j in coords:
                    ...
                    Es = np.array(
                        [self.mrf.energy_pixel(v, i, j, c, Y) for v in candidates]
                    )
                    logp = -Es
                    logp -= logp.max()
                    p = np.exp(logp)
    ...
    def fit_optimised(
        self, shuffle_pixels=False, parallel_channels=False, binary_map=False
    ):
                    ...
                    if binary_map:
                        ...
                        for idx in range(m):
                            ...
                            if self.pior_type_for_optimal == "quadratic":
                                ...
                                energies[idx] = mrf_c.lambda_r * (
                                    v - x_obs
                                ) ** 2 + self.beta_prior * np.sum(truncated)
                            elif self.pior_type_for_optimal == "potts":
                                energies[idx] = mrf_c.lambda_r * (
                                    v - x_obs
                                ) ** 2 + self.beta_prior * np.sum(neigh != v)
                            else:
                                raise ValueError(
                                    f"Unknown prior type: {self.pior_type_for_optimal}"
                                )
                        e0 = energies.min()
        ...
        if C == 1 or binary_map:
            ...
            if parallel_channels:
                with ThreadPoolExecutor(max_workers=C) as ex:
                    results = [
                        f.result() for f in [ex.submit(worker, c) for c in range(C)]
                    ]
            else:
