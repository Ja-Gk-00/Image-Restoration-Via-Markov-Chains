...
def mape_sa_quadratic(
    ...
    output_file_mape_sa_quad = (
        output_folder / f"{image_path.stem}_mape_sa_quadratic{image_path.suffix}"
    )
    cv2.imwrite(
        str(output_file_mape_sa_quad),
        cv2.cvtColor(denoised_mape_sa_quad.astype(np.uint8), cv2.COLOR_RGB2BGR),
    )
...
def mape_sa_potts(
    ...
    output_file_mape_sa_potts = (
        output_folder / f"{image_path.stem}_mape_sa_potts{image_path.suffix}"
    )
    cv2.imwrite(
        str(output_file_mape_sa_potts),
        cv2.cvtColor(denoised_mape_sa_potts.astype(np.uint8), cv2.COLOR_RGB2BGR),
    )
...
def mmse_quadratic(
    output_folder: Path, image_path: Path, num_iter: int, burn_in: int, mrf: GridMRF
):
    ...
    output_file_mmse_quad = (
        output_folder / f"{image_path.stem}_mmse_quadratic{image_path.suffix}"
    )
    cv2.imwrite(
        str(output_file_mmse_quad),
        cv2.cvtColor(denoised_mmse_quad.astype(np.uint8), cv2.COLOR_RGB2BGR),
    )
...
def mmse_potts(
    ...
    output_file_mmse_potts = (
        output_folder / f"{image_path.stem}_mmse_potts{image_path.suffix}"
    )
    cv2.imwrite(
        str(output_file_mmse_potts),
        cv2.cvtColor(denoised_mmse_potts.astype(np.uint8), cv2.COLOR_RGB2BGR),
    )
...
def generate_and_save_denoised_images(image_path_str: str, output_folder_str: str):
    ...
    mrf = GridMRF(noisy, loss, prior, lambda_r=1, window_term=None, lambda_w=1)
    ...
    tasks = [
        (
            "mape_sa_quad",
            mape_sa_quadratic,
            (output_folder, image_path, num_iter, burn_in, mrf, betas_sa),
        ),
        (
            "mape_sa_potts",
            mape_sa_potts,
        ...
        (
            "mmse_quad",
            mmse_quadratic,
            (output_folder, image_path, num_iter, burn_in, mrf),
        ),
